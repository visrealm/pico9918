...
software, swept across two line-width tiers, plus seven property suites that compute the answer
...
Two of the seven are not about pixels at all. `test_gpu_dma` runs the GPU's DMA engine, and it is
the only thing in either repository that makes a board's DMA trigger fire - `gpu/` below holds the
same geometry as a host binary with no board build, so it never has. `test_tms9900` uploads programs
and runs them, which is the only way an instruction reaches the core a board runs *as firmware*:
`tms9900/` below reaches the same assembly core, but as a UF2 that replaces the firmware, so it says
nothing about the GPU loop, the budget or the MPU around it. Both are cases with computed answers
rather than pictures, so a board draws a scoreboard while it works through them.
