...
def rom_size_kb(max_bank, bank_kb):
    """Smallest CVBasic ROM size holding banks 0..max_bank.

    The TI-99 (8KB banks) keeps its own banks 0-2 as fixed space and shifts
    every bank we ask for up by two.
    """
    banks = max_bank + 1 + (2 if bank_kb == 8 else 0)
    for kb in (128, 256, 512, 1024):
        if banks * bank_kb <= kb:
            return kb
    raise SystemExit("Firmware needs {0}KB - CVBasic tops out at 1024KB".format(banks * bank_kb))


def write_banksel(path, max_bank, bank_kb):
    """Emit a banksel.bas with the ROM size and thunks for banks 0..max_bank.

    Bank 0 is the dispatcher (only bank that may issue BANK SELECT). Bank 1
    holds menu-palette.bas. Banks 2..max_bank hold firmware payload data.
    Each selectBankN procedure is a single BANK SELECT N - we need one per
    bank because BANK SELECT requires a literal bank number in CVBasic.
    """
    with open(path, mode='w') as f:
        f.write("' Auto-generated by uf2cvb.py - do not edit.\n")
        f.write("' Provides BANKSEL(n) for banks 0..{0}.\n\n".format(max_bank))
        f.write("#if BANK_SIZE\n\n")
        if bank_kb:
            f.write("    BANK ROM {0}\n\n".format(rom_size_kb(max_bank, bank_kb)))
        f.write("g_currentBank = 0\n\n")
        f.write("    DEF FN BANKSEL(BANK) = g_currentBank = BANK : GOSUB selectBank\n\n")
        for b in range(0, max_bank + 1):
            f.write("selectBank{0}: PROCEDURE\n".format(b))
            f.write("    BANK SELECT {0}\n".format(b))
            f.write("    END\n\n")
        f.write("selectBank: PROCEDURE\n")
        thunks = ",".join("selectBank{0}".format(b) for b in range(0, max_bank + 1))
        f.write("    ON g_currentBank FAST GOSUB {0}\n".format(thunks))
        f.write("    END\n\n")
        f.write("#else   ' No banking\n")
        f.write("    DEF FN BANKSEL(BANK) = B = BANK\n")
        f.write("#endif\n")
...
def main() -> int:
    ...
    parser.add_argument('--banksel', help='also write a banksel.bas with thunks sized for this firmware', default=None)
    ...
    if is_combined:
        ...
        # RP2350 .uf2s also carry a picotool absolute-family block the firmware
        # writer rejects, so take the family we recognise rather than the first.
        single_blocks = rp2040_blocks or rp2350_blocks or all_blocks
    ...
    with open(args['outfile'] + ".h.bas", mode='w') as header:
        with open(args['outfile'] + ".bas", mode='w') as output:
            ...
            if is_combined:
                ...
                max_bank = rp2350_start_bank + banks_2350 - 1
            else:
                write_blocks_section(output, single_blocks, 2, BLOCKS_PER_BANK, use_banking, 0)
                max_bank = 2 + total_banks - 1

    if args['banksel']:
        # without banking, max_bank is unused - emit the shim form regardless
        write_banksel(args['banksel'],
                      max_bank if use_banking else 0,
                      args['banksize'] if use_banking else 0)
        print("Banksel file    : {0}".format(args['banksel']))
