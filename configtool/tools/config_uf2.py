...
def create_default_config(pico_model=PICO_MODEL_RP2040,
    ...
    config[PICO9918_CONF_CRT_SCANLINES] = scanlines & 0x01
    config[PICO9918_CONF_SCANLINE_SPRITES] = scanline_sprites & 0x03
    config[PICO9918_CONF_CLOCK_PRESET_ID] = clock_preset & 0x03

    # Set diagnostics to off
    config[PICO9918_CONF_DIAG] = 0
    config[PICO9918_CONF_DIAG_REGISTERS] = 0
    config[PICO9918_CONF_DIAG_PERFORMANCE] = 0
    config[PICO9918_CONF_DIAG_PALETTE] = 0
    config[PICO9918_CONF_DIAG_ADDRESS] = 0

    # Set palette
    palette = custom_palette if custom_palette else DEFAULT_PALETTE

    # Palette entry 0 is always 0x0000 (black)
    config[PICO9918_CONF_PALETTE_IDX_0] = 0x00
    config[PICO9918_CONF_PALETTE_IDX_0 + 1] = 0x00

    # Palette entries 1-15 use default TMS9918A colors with 0xF alpha
    for i in range(1, 16):
        rgb = palette[i]
        # Store as big-endian (high byte first)
        config[PICO9918_CONF_PALETTE_IDX_0 + (i * 2)] = (rgb >> 8) & 0xFF
