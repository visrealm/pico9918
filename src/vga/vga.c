/**
 * \file
 * \brief VGA and RGBs output - PIO sync/rgb generation and the core 1 render loop
 *
...
/** \brief round to the nearest int, avoiding a dependency on math.h */
int __in_flash_func(roundflt)(float x)
{
  if (x < 0.0f)
    return (int)(x - 0.5f);
  else
...
/** \brief sync pio dma data buffers, one full line each */
uint32_t __aligned(8) syncDataActive[4]; // active display line
uint32_t __aligned(8) syncDataPorch[4];  // porch/blanking line
uint32_t __aligned(8) syncDataSync[4];   // VGA vsync line, non-interlaced only

/** \brief interlaced PAL/NTSC vsync buffers, each a full line of two half-line pulses
 *  \note  every sync transfer is 4 words, so the dma count never changes and the fifo never starves
 */
uint32_t __aligned(4) syncDataLsLs[4]; // LS + LS
uint32_t __aligned(4) syncDataLsEq[4]; // LS + EQ
uint32_t __aligned(4) syncDataEqEq[4]; // EQ + EQ
uint32_t __aligned(4) syncDataEqLs[4]; // EQ + LS, the interlace transition line

__attribute__((section(".scratch_y.lookup"))) uint16_t __aligned(4) rgbDataBuffer[2][RGB_PIXELS_X] = {
  0}; // two scanline buffers (odd and even)
 ...
 */
/*
 * DMA channels driving the display. These are the ONLY declaration of the numbers -
 * both the claim and the channel variables below derive from them.
 *
 * Fixed, not dma_claim_unused_channel(): the display's two channels are reserved by
 * number at vgaInit (VGA_DMA_CLAIM) so any collision panics, which is the whole
 * guarantee an allocator would have bought. Allocating them instead would make WHICH
 * channel carries video depend on init order - and the library claims its own three
 * from pico9918_init(), which runs first (main.c). Keeping the numbers fixed and
 * claimed makes that ordering irrelevant, which is the point.
 */
#define SYNC_DMA_CHAN 0u
#define RGB_DMA_CHAN  1u
#define VGA_DMA_CLAIM() dma_claim_mask((1u << SYNC_DMA_CHAN) | (1u << RGB_DMA_CHAN))

static int syncDmaChan = SYNC_DMA_CHAN;
static int rgbDmaChan = RGB_DMA_CHAN;
static uint syncDmaChanMask = 0;
static uint rgbDmaChanMask  = 0;
static VgaInitParams __uninitialized_ram(vgaParams);
static pio_sm_config rgbConfig;
uint rgbProgOffset;

/** \brief slowest PIO clock a mode can be generated at, in kHz */
static uint32_t vgaMinimumPioClockKHz(const VgaParams* params)
{
  return params->pixelClockKHz * vga_rgb_LOOP_TICKS / params->hPixelScale;
}


/** \brief VSYNC_* line type to sync buffer, populated by buildSyncData() for interlaced modes */
static const uint32_t* vsyncTypeBuffers[VSYNC_TYPE_COUNT];

/** \brief build the sync dma buffers for the current mode, false if the system clock is too slow */
static bool __in_flash_func(buildSyncData)(void)
{
  ...
  vgaParams.params.pioClocksPerScaledPixel =
    vgaParams.params.pioFreqKHz * vgaParams.params.hPixelScale / (float)vgaParams.params.pixelClockKHz;

  const uint32_t activeTicks =
    roundflt(vgaParams.params.pioClocksPerPixel * (float)vgaParams.params.hSyncParams.displayPixels) -
    vga_sync_SETUP_OVERHEAD;
  const uint32_t fPorchTicks =
    roundflt(vgaParams.params.pioClocksPerPixel * (float)vgaParams.params.hSyncParams.frontPorchPixels) -
    vga_sync_SETUP_OVERHEAD;
  const uint32_t syncTicks =
    roundflt(vgaParams.params.pioClocksPerPixel * (float)vgaParams.params.hSyncParams.syncPixels) -
    vga_sync_SETUP_OVERHEAD;
  const uint32_t bPorchTicks =
    roundflt(vgaParams.params.pioClocksPerPixel * (float)vgaParams.params.hSyncParams.backPorchPixels) -
    vga_sync_SETUP_OVERHEAD;
  ...
  // interlaced: the EQ pulse comes from shortPulsePixels, the guard fills the rest of the half-line
  if (vgaParams.params.interlaced)
  {
    ...
    // EQ+LS: [27px LOW][405px HIGH][405px LOW][27px HIGH] = 864px - interlace transition line
...
/** \brief initialise the vga sync pins, pio program and dma */
static void __in_flash_func(vgaInitSync)(void)
{
  buildSyncData();
...
/** \brief initialise the vga rgb pins, pio program and dma */
static void __in_flash_func(vgaInitRgb)(void)
{
  ...
  pio_program_t rgbProgram = {
    .instructions = rgbProgramInstr, .length = vga_rgb_program.length, .origin = vga_rgb_program.origin};
  ...
  // the extra word of zeros absorbs the PIO's speculative autopull, so no stale fifo data
  // bleeds into the next scanline. those 2 pixels land in the blanked front porch
  ...
  sm_config_set_out_shift(&rgbConfig, true, true, 32); // R shift, autopull @ 32 bits (2 pixels)
  ...
  channel_config_set_transfer_data_size(&rgbDmaChanConfig,
                                        DMA_SIZE_32);           // transfer 32 bits at a time (2 pixels)
  ...
  dma_channel_configure(rgbDmaChan, &rgbDmaChanConfig, &VGA_PIO->txf[RGB_SM], rgbDataBuffer[0],
                        vgaParams.params.hSyncParams.displayPixels / 2 + 1, false);
  dma_channel_set_irq0_enabled(rgbDmaChan, true);
}

/** \brief dma interrupt handler: queue each line's sync and rgb buffers, and post work to core 1 */
static void __isr __time_critical_func(dmaIrqHandler)(void)
{
  ...
  if (dma_hw->ints0 & syncDmaChanMask)
  {
    ...
    if (vgaParams.params.interlaced)
    {
      ...
      if (currentLine < vsyncEnd)
      {
        ...
        dma_channel_set_read_addr(syncDmaChan, vsyncTypeBuffers[field->vsyncPattern[currentLine]], true);
      }
      ...
      else
      {
        ...
        dma_channel_set_read_addr(syncDmaChan, vsyncTypeBuffers[field->trailingPattern[trailingIdx]], true);
    ...
    else
    {
      ...
      else
      {
        dma_channel_set_read_addr(syncDmaChan, syncDataPorch, true);
        if (currentLine ==
            (vgaParams.params.vSyncParams.totalPixels - vgaParams.params.vSyncParams.frontPorchPixels) + 2)
        {
          multicore_fifo_push_timeout_us(FRONT_PORCH_MSG, 0);
        }
      }
  ...
  if (dma_hw->ints0 & rgbDmaChanMask)
  {
...
#if PICO_RP2040
    pio_sm_set_pindirs_with_mask(VGA_PIO, RGB_SM, (vgaParams.scanlines && (currentDisplayLine & 0x01)) - 1,
                                 (1 << 5) | (1 << 9) | (1 << 13));
#endif
    ...
    if (pxLine == vgaParams.triggerScanline && pxLineRpt == vgaParams.params.vPixelScale - 1)
    {
      multicore_fifo_push_timeout_us(END_OF_SCANLINE_MSG | pxLine, 0);
    }
  }

  /* core 0 owns the MPU the GPU faults on, and this is the only place that core is
     idle with a scanline's regularity - last, so it is never in front of the DMA */
  if (pico9918_gpu_palette_guard_off) pico9918_gpu_rearm_palette_guard();
}

/** \brief hook the dma irq and start both channels */
static void __in_flash_func(initDma)(void)
{
...
/** \brief core 1 render loop */
void __time_critical_func(vgaLoop)(void)
{
...
/** \brief initialise the vga and start output */
void __in_flash_func(vgaInit)(VgaInitParams params)
{
  vgaParams = params;

  /* reserve the display's channels before either init configures one */
  VGA_DMA_CLAIM();
